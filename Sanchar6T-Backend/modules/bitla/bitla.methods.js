...
const normalized = {
  ...
  after:
    filters.departureAfter || null,

  before:
    filters.departureBefore || null,

  afterArrival:
    filters.arrivalAfter || null,
...
//   const wanted = String(filters.seatType).toLowerCase();
...
applyFilters(schedules, filters) {
  ...
  return schedules.filter((bus) => {
    ...
    if (filters.rating) {
      ...
      const busRating =
        Number(
          bus.service_rating || 0
        );

      if (busRating < wantedRating) {
        return false;
      }
    }
...
if (filters.departure) {
  ...
  if (busTime !== selectedTime) {
    ...
    if (filters.cancellable !== undefined) {
      ...
      const wants =
        filters.cancellable === "true" ||
        filters.cancellable === true;

      if (wants !== bus.is_cancellable) {
        return false;
      }
    }
    ...
    if (filters.pickup) {
      ...
      const hasMatch =
        availablePickupIds.some((id) =>
          selectedPickupIds.includes(id)
        );
    ...
    if (filters.dropoff) {
      ...
      const hasMatch =
        availableDropoffIds.some((id) =>
          selectedDropoffIds.includes(id)
        );
